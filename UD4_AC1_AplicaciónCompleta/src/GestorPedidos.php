...
class GestorPedidos
{
    ...
    {
        if (self::$pdo === null) self::$pdo = Conexion::getConexion();
        return self::$pdo;
    }
    ...
    public static function getOrCreatePedidoAbierto(string $codRes): string
    {
        ...
        if ($abierto) return (string)$abierto['CodPed'];
    ...
    public static function getCarritoPorCorreo(string $correo): array
    {
        ...
        if (!$codRes) return ['pedido' => null, 'lineas' => []];

        $pedido = self::getPedidoAbierto($codRes);
        if (!$pedido) return ['pedido' => null, 'lineas' => []];
    ...
    {
        $codRes = self::getCodResByCorreo($correo);
        if (!$codRes) throw new \RuntimeException("Restaurante no encontrado.");
        ...
        if ($nuevo > $stock) $cantidad = max(0, $stock - $actual);
    ...
    public static function enviarCarritoPorCorreo(string $correo): void
    {
        ...
        $pedido = self::getPedidoAbierto($codRes);
        if (!$pedido) throw new \RuntimeException("El carrito está vacío.");

        // ✅ ID real del pedido
        $idPedido = (string)($pedido['CodPed'] ?? '');
        if ($idPedido === '') throw new \RuntimeException("No se pudo determinar el ID del pedido.");

        // ✅ TO = correo de la sesión (si existe), si no, el parámetro
        $correoRestaurante = $_SESSION['correo'] ?? $correo;
        $cc = 'pedidos@caraculo.com';

        // ✅ Fecha real del pedido (en tu tabla es DATE)
        $fecha = (string)($pedido['Fecha'] ?? date('Y-m-d'));

        // ✅ Cargar líneas desde TU BD (pedidosproductos + productos)
        $db = Conexion::getConexion(); // ajusta si tu clase se llama distinto
        $sql = "SELECT
        ...
        // (Opcional) marcar pedido como enviado/cerrado (tu lógica)
